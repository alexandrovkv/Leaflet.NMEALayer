/*
 * nmea.js - NMEA 0183  parser/encoder
 *
 */
...
var NMEA = function() {
	...
	var M = parseInt(date.slice(2, 4), 10) - 1;
    ...
    /** GPVTG parser object */
    nmea.VtgParser = function(id) {
	this.id = id;
	this.parse = function(tokens) {
            var vtg;

            if(tokens.length < 9) {
		nmea.error('VTG : not enough tokens');
		return null;
            }
		...
		magdev:  nmea.parseFloatX(tokens[3]),
		sogN:    nmea.parseFloatX(tokens[5]),
		sogK:    nmea.parseFloatX(tokens[7])
            };
    ...
    /** GPGLL parser object */
    nmea.GllParser = function(id) {
	this.id = id;
	this.parse = function(tokens) {
            var gll;

            if(tokens.length < 7) {
		nmea.error('GLL : not enough tokens');
		return null;
            }
    ...
    /** GPZDA parser object */
    nmea.ZdaParser = function(id) {
	this.id = id;
	this.parse = function(tokens) {
            var zda;

            if(tokens.length < 7) {
		nmea.error('ZDA : not enough tokens');
		return null;
            }
		...
		id:       tokens[0].substr(1),
		time:     tokens[1],
		day:      nmea.parseIntX(tokens[2],10),
		month:    nmea.parseIntX(tokens[3],10),
		year:     nmea.parseIntX(tokens[4],10),
		offset:   nmea.parseIntX(tokens[5],10),
		offsetM:  nmea.parseIntX(tokens[6],10),
            };
	...
	this.parse = function(tokens) {
	    ...
	    var i;
            ...
            if(tokens.length < 18) {
		nmea.error('GSA : not enough tokens');
		return null;
            }
		...
		id:    tokens[0].substr(1),
		mode:  tokens[1],
		fix:   nmea.parseIntX(tokens[2],10),
		prns:  [],
		...
		vdop:  nmea.parseFloatX(tokens[17])
            ...
            for(i=3;i<15;i++) {
		sat = nmea.parseIntX(tokens[i],10);
		gsa.prns.push(sat);
            }
	...
	if(id.length !== 5) {
            this.error('ID must be exactly 5 characters ' + id);
            return null;
	}
	...
	if(result === null) {
            this.error('sentence id not found ' + id);
	}
    ...
    nmea.addParser(new nmea.VtgParser("GPVTG"));
    nmea.addParser(new nmea.GsaParser("GPGSA"));
    nmea.addParser(new nmea.GllParser("GPGLL"));
    nmea.addParser(new nmea.ZdaParser("GPZDA"));
